...
class RomanParser (Parser):
    ...
    def __init__(self, schema):
        ...
        self.__alphabet = schema.get_config_char_sequence(
            u'Alphabet') or u'abcdefghijklmnopqrstuvwxyz'
        self.__delimiter = schema.get_config_char_sequence(
            u'Delimiter') or u' '
        self.__max_keyword_length = int(
            schema.get_config_value(u'MaxKeywordLength') or u'7')
        get_rules = lambda f, key: [f(r.split())
                                    for r in schema.get_config_list(key)]
        ...
        if self.__use_keyword_mapping:
            ...
            d = dict([reduce(apply_spelling_rule, spelling_rules, (k, k))
                      for k in keywords])
    ...
    def __parse(self, ctx):
        ...
        while i < n:
            ...
            if not keyword:
                ...
                break
        ctx.aux = lambda p: u''.join(
            [self.__delimiter[0] if x is None else x for x in k[p * 2:]] + [remainder])
        if self.__use_keyword_mapping:
...
class ComboParser (RomanParser):
    ...
    def __init__(self, schema):
        ...
        self.__combo_keys = schema.get_config_char_sequence(
            u'ComboKeys') or u''
        self.__combo_codes = schema.get_config_char_sequence(
            u'ComboCodes') or u''
        self.__combo_max_length = min(
            len(self.__combo_keys), len(self.__combo_codes))
