...
def __initialize():
    ...
    if not os.path.exists(user_db):
        sys_db = IBUS_ZIME_LOCATION and os.path.join(
            IBUS_ZIME_LOCATION, 'data', 'zime.db')
            ...
            if mask & ~modifier.RELEASE_MASK == modifier.CONTROL_MASK and keycode >= keysyms._1 and keycode <= keysyms._9:
                ...
                if candidates:
                    if mask & modifier.RELEASE_MASK == 0:
                        # delete phrase
                        index = self.__frontend.get_candidate_index(
                            keycode - keysyms._1)
        ...
        if self.__punct:
            if keycode not in (keysyms.Shift_L, keysyms.Shift_R) and not (mask & modifier.RELEASE_MASK):
                if keycode == self.__punct_key:
                    # next punct
                    self.__punct_rep = (
                        self.__punct_rep + 1) % len(self.__punct)
...
class SchemaChooser:
    ...
    def __load_schema_list(self):
        ...
        self.__schema_list = [(x[1], x[0]) for x in sorted(
            s, key=last_used_time, reverse=True)]
    ...
    def choose(self, schema_name):
        ...
        if c != -1:
            now = time.time()
            DB.update_setting(u'SchemaChooser/LastUsed/%s' %
                              s[c], unicode(now))
            self.__deactivate()
