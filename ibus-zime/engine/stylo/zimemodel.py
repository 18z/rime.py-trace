...
join_phrase = lambda w: (u'' if all(
    [len(x) == 1 for x in w]) else u' ').join(w)
...
class Model:
    ...
    def __concatenated(self, ctx, pos, x):
        for i in range(pos):
            ...
            for y in c[j]:
                if y[0][-3:] == x[0][:3]:
                    self.__add_candidate(
                        ctx, i, j + 2, (y[0] + [x[0][-1]], min(y[1], x[1]), min(y[2], x[2])))
                    ok = True
            if ok:
                return True
        return False
    ...
    def delete_phrase(self, ctx, c):
        ...
        if len(keywords) <= 4:
            ...
            for i in range(len(keywords) - 4 + 1):
                db.delete(keywords[i:i + 4],
                          join_phrase(words[i:i + 4]), c[2][2])
        # update context
        j = 0
