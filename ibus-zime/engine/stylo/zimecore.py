...
class Schema:
    ...
    def __init__(self, name):
        ...
        self.__auto_prompt = self.__db.read_config_value(
            u'AutoPrompt') == u'yes'
...
class Parser:
    ...
    def __init__(self, schema):
        ...
        punct_mapping = lambda (x, y): (x, (0, y.split(u' ')) if u' ' in y else
                                           ...
                                           (1, y))
        self.__punct = dict([punct_mapping(c.split(None, 1))
                             for c in schema.get_config_list(u'Punct')])
        key_mapping = lambda (x, y): (
            keysyms.name_to_keycode(x), keysyms.name_to_keycode(y))
        self.__edit_keys = dict([key_mapping(c.split(None, 1))
                                 for c in schema.get_config_list(u'EditKey')])
