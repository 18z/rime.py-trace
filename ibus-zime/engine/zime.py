...
class ZimeEngine (ibus.EngineBase):
    ...
    def update_preedit(self, s, start, end):
        ...
        preedit_attrs.append(ibus.AttributeUnderline(
            ibus.ATTR_UNDERLINE_SINGLE, 0, length))
        if start < end:
            preedit_attrs.append(ibus.AttributeBackground(
                ibus.RGB(0, 0, 0), start, end))
            preedit_attrs.append(ibus.AttributeForeground(
                ibus.RGB(255, 255, 255), start, end))
        super(ZimeEngine, self).update_preedit_text(
            ibus.Text(s, preedit_attrs), length, True)
